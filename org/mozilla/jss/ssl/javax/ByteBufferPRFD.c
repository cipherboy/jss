...
// Define a stubbed capacity value as a j_bytebuffer doesn't indicate an
// underlying capacity, unlike j_buffer.
#define STUB_CAPACITY 0
...
static PRStatus PRByteBufferGetSocketOption(PRFileDesc *fd, PRSocketOptionData *data)
{
    ...
    case PR_SockOpt_MaxSegment:
        data->value.max_segment = STUB_CAPACITY;
        return PR_SUCCESS;
    case PR_SockOpt_RecvBufferSize:
        data->value.recv_buffer_size = STUB_CAPACITY;
        return PR_SUCCESS;
    case PR_SockOpt_SendBufferSize:
        data->value.send_buffer_size = STUB_CAPACITY;
        return PR_SUCCESS;
    default:
        return PR_FAILURE;
    }
 ...
 * underlying j_bytebuffers and does not free them; it is up to the caller to
 * do so. */
void freeByteBufferPRFileDesc(PRFileDesc *fd)
{
    ...
    uint8_t *peer_info, size_t peer_info_len)
{
    ...
    if (fd) {
        fd->methods = &PRIOByteBufferMethods;
        ...
        fd->lower = NULL;
        fd->higher = NULL;
        fd->dtor = freeByteBufferPRFileDesc;
    }
