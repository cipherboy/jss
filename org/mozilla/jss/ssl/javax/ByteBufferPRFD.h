...
/* Free a ByteBuffer-backed PRFileDesc. Note that it is usually sufficient to
 * call PR_Close(...) on the buffer instead. This is provided for completeness
 * and should not be called in a SSL context as the buffer PRFileDesc is
 * wrapped by the SSL PRFileDesc. Note that this only removes references to
 * the underlying j_bytebuffers and does not free them; it is up to the caller
 * to do so. */
